...
tf.flags.DEFINE_integer("render-every", "10", "Render environment every n episodes")
...
# Open file to log
assert FLAGS.logfile is not None
...
env.seed(SEED)
env = wrappers.Monitor(env, '/tmp/mountain-car-exp-1')
...
def actor_critic(env, estimator_policy, estimator_value, num_episodes, discount_factor=1.0):
    ...
    for i_episode in trange(num_episodes, desc="Training Progress ", position=1):
        ...
        for t in pbar:
            ...
            if (i_episode + 1) % FLAGS.render_every == 0:
                env.render()
...
with tf.Session() as sess:
    ...
    stats = actor_critic(env, policy_estimator, value_estimator, num_episodes=50000, discount_factor=0.95)
