...
tf.flags.DEFINE_integer("max-episodes", "1000", "Maximum steps per episode")
tf.flags.DEFINE_integer("max-steps", "200", "Maximum steps per episode")
...
tf.flags.DEFINE_boolean("random-learning-rate", True, "Random sample learning rate from LogUniform(min, max)")
...
def actor_critic(env, estimator_policy, estimator_value, num_episodes, discount_factor=1.0):
    ...
    for i_episode in trange(num_episodes, desc="Training Progress ", position=1):
        ...
        if i_episode >= 100 and np.mean(stats.episode_rewards[i_episode-100:i_episode]) > -110:
